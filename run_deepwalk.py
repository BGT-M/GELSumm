...
def test(embeds):
    R, S, adj, adj_s, features, labels, full_labels, full_indices = load_data(
        args.dataset)
    adj = normalize(adj)
    del R, adj_s, labels, full_indices, features

    embeds = S @ embeds
    for _ in range(2):
        embeds = adj @ embeds
    embeds = to_torch(embeds)
    predict = F.log_softmax(embeds, dim=1)
    loss_test = F.nll_loss(predict, full_labels)
    acc_test = accuracy(predict, full_labels)
    f1_micro, f1_macro = f1(predict, full_labels)
    logger.info(
